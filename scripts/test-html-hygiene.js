...
assert.ok(html.includes('<meta name="application-name" content="智慧教务管理系统">'), 'index.html should expose the application name to browsers');
...
assert.ok(html.includes('<meta property="og:site_name" content="智慧教务管理系统">'), 'index.html should include readable Open Graph site name');
assert.ok(html.includes('<meta name="twitter:title" content="智慧教务管理系统">'), 'index.html should include readable Twitter metadata');
assert.ok(html.includes('<meta name="apple-mobile-web-app-title" content="智慧教务">'), 'index.html should expose readable iOS app title');
...
assert.ok(html.includes('./assets/css/product-redesign.css?v=20260606-product-redesign-v1'), 'index.html should load the product redesign layer after legacy styles');
...
assert.ok(html.includes("var refreshVersion = '20260606-product-redesign-v1';"), 'early runtime refresh version should match the service worker runtime');
