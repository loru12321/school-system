...
function run() {
    const root = {
        ...
        MANUAL_ID_MAPPINGS: { 'A_1_Alice': '1班' },
        LAST_VA_DATA: [{ name: '一班', valueAdded: 8.2 }],
        VA_VIEW_MODE: 'class',
    ...
    assert.deepStrictEqual(progressState.getManualIdMappings(), { 'A_1_Alice': '1班' });
    assert.deepStrictEqual(progressState.getLastVaData(), [{ name: '一班', valueAdded: 8.2 }]);
    ...
    const snapshot = progressState.syncProgressState({
        ...
        lastVaData: [{ name: '二班', valueAdded: 3.4 }],
        vaViewMode: 'school',
        quickMode: 'my_class'
    });
    ...
    assert.deepStrictEqual(snapshot.lastVaData, [{ name: '二班', valueAdded: 3.4 }]);
