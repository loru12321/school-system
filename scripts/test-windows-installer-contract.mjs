...
const exePath = path.join(root, 'public/downloads', windowsAsset.assetName);
const requireLocalReleaseAssets = process.env.REQUIRE_LOCAL_RELEASE_ASSETS === 'true';
const hasLocalInstaller = existsSync(exePath);
...
if (hasLocalInstaller) {
  ...
  assert.ok(!requireLocalReleaseAssets, 'Windows local installer is required for release packaging verification');
}

const hasNativeBuild = existsSync(path.join(root, 'native-builds/windows/win-unpacked/resources/app/index.html'))
  && existsSync(path.join(root, 'native-builds/windows/win-unpacked/resources/app/assets/js/app.js'))
  && existsSync(path.join(root, 'native-builds/windows/win-unpacked/school-system.exe'))
  && existsSync(path.join(root, 'native-builds/windows/school-system-windows-1.0.2-x64.exe'));
assert.ok(hasNativeBuild || !requireLocalReleaseAssets, 'unpacked Windows build must include bundled local app resources');
