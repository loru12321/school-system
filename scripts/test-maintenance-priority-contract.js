...
const guardedItems = [
  ...
  () => assertIncludes(headers, '/assets/js/*\n  Cache-Control: public, max-age=3600, stale-while-revalidate=86400', 'unversioned runtime JS should use bounded browser caching'),
  ...
  () => assertIncludes(sw, 'isVersionedRuntimeAsset(url)', 'service worker should recognize immutable runtime versions'),
  () => assertIncludes(sw, 'cacheFirstRuntimeAsset(request)', 'service worker should cache-first immutable runtime assets'),
