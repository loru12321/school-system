...
assertIncludes(publicSw, 'event.respondWith(cacheFirstStatic(request));', 'static assets should use cache-first handling');
...
assertIncludes(publicSw, "headers: { 'Content-Type': 'application/json' }", 'offline API fallback should be JSON');
assertIncludes(publicSw, "headers: { 'Content-Type': 'text/html; charset=utf-8' }", 'offline HTML fallback should be HTML with charset');
