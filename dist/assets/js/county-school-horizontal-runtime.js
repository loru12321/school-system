(()=>{if(typeof window=="undefined"||window.CountySchoolHorizontalRenderer)return;function o(t,l,n){return n?t&&typeof t.sameSchoolName=="function"?t.sameSchoolName(l,n):String(l||"").trim()===String(n||"").trim():!1}function d(t,l=""){const n=t.buildCountyHorizontalTotalRows();if(!n.length)return'<div class="county-empty">暂无学校成绩数据，请先导入本次县级成绩。</div>';const s=n.reduce((a,e)=>Math.max(a,t.toNumber(e.avg)),0)||100;return`
            <div class="table-wrap analysis-table-shell">
                <table class="analysis-generated-table county-analysis-table" data-virtual-table="county-horizontal-total">
                    <thead>
                        <tr>
                            <th>学校名称 <span class="analysis-table-tag">共识别 ${n.length} 所</span></th>
                            <th>实考人数</th>
                            <th>平均分</th>
                            <th>优秀率</th>
                            <th>及格率</th>
                    ...
                    <tbody>
                        ${n.map(a=>{const e=o(t,a.schoolName,l),u=a.avg?Math.min(100,a.avg/s*100).toFixed(1):0;return`
                                <tr class="${e?"bg-highlight":""}">
                                    <td data-label="学校名称">${t.escapeHtml(a.schoolName)}</td>
                                    <td data-label="实考人数">${a.count||0}</td>
                                    <td data-label="平均分" class="data-bar-bg" style="--percent:${u}%">${t.formatCountyRankDisplay(a.avg,a.rankAvg)}</td>
        ...
        `}function i(t,l,n){return l.map(s=>{const a=t.buildCountySubjectRows(s);return a.length?`
                <div class="analysis-anchor-panel county-subject-detail">
                    ...
                    <div class="table-wrap analysis-table-shell">
                        <table class="analysis-generated-table county-analysis-table" data-virtual-table="county-subject-${t.escapeHtml(s)}">
                            ...
                            <tbody>
                                ${a.map(e=>`
                                    <tr class="${o(t,e.schoolName,n)?"bg-highlight":""}">
            ...
            `:""}).filter(Boolean).join("")}function r(t){const l=t.buildCountyHorizontalTotalRows();if(!l.length)return'<div class="county-empty">暂无学校成绩数据，请先导入本次县级成绩。</div>';const n=t.sortCountySubjects(window.SUBJECTS||[]),s=t.resolveCurrentCountySchoolName(),a=i(t,n,s);return`
            ...
            <div class="county-kpi-grid">
                ...
                <div><span>学科明细</span><strong>${n.length}</strong><em>按两率一分统一折算</em></div>
        ...
        `}window.CountySchoolHorizontalRenderer={renderTotalTable:d,renderSchoolHorizontal:r}})();
