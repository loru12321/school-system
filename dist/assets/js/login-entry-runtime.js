(()=>{if(typeof window=="undefined"||window.__LOGIN_ENTRY_RUNTIME_PATCHED__)return;const O={school:{userPrefix:"User",userPlaceholder:"管理员账号 / 教师姓名",passPrefix:"Password",submit:"进入学校工作台",helper:"当前为学校端，验证通过后进入届别选择和工作台。"},parent:{userPrefix:"Student",userPlaceholder:"请输入学生姓名",passPrefix:"Password",submit:"家长端登录",helper:"家长端统一承接原学生端与家长端入口：填写学生姓名、班级和密码。首次使用默认密钥会被强制修改。"}};function q(){return window.Auth&&typeof window.Auth.getLoginPortal=="function"?window.Auth.getLoginPortal()==="parent"?"parent":"school":localStorage.getItem("LOGIN_PORTAL_V1")==="parent"?"parent":"school"}function A(t,e,o){if(!t)return null;let a=t.closest(".login-entry-field");a||(a=document.createElement("div"),a.className="login-entry-field",a.dataset.loginField=e,t.parentNode.insertBefore(a,t),a.appendChild(t));let n=a.querySelector(".login-entry-prefix");return n||(n=document.createElement("span"),n.className="login-entry-prefix",a.insertBefore(n,a.firstChild)),n.textContent!==o&&(n.textContent=o),a}function k(t=new Date){return String($(t)-3)}function $(t=new Date){return t.getMonth()>=8?t.getFullYear():t.getFullYear()-1}const at=4;function N(t=new Date){const e=Number(k(t)),o=[];for(let a=0;a<at;a+=1)o.push(String(e+a));return o}const I="LOGIN_GRADUATE_COHORT_TARGET_V1",P="LOGIN_SELECTED_COHORT_TARGET_V1";function H(t=new Date){const e=Number(k(t)),o=new Set;for(let a=1;a<=6;a+=1){const n=e-a;n>=2e3&&o.add(String(n))}try{const a=JSON.parse(localStorage.getItem("COHORT_LIST")||"[]");(Array.isArray(a)?a:[]).forEach(n=>{var s;const l=((s=String((n==null?void 0:n.id)||(n==null?void 0:n.year)||"").match(/\d{4}/))==null?void 0:s[0])||"";l&&Number(l)<e&&o.add(l)})}catch(a){}return Array.from(o).sort((a,n)=>Number(n)-Number(a))}function D(){try{const t=sessionStorage.getItem(I)||"";return/^\d{4}$/.test(t)?t:""}catch(t){return""}}function x(t){const e=String(t||"").trim();try{/^\d{4}$/.test(e)?sessionStorage.setItem(I,e):sessionStorage.removeItem(I)}catch(o){}return/^\d{4}$/.test(e)?e:""}function G(){try{const t=sessionStorage.getItem(P)||"";return/^\d{4}$/.test(t)?t:""}catch(t){return""}}function W(t){const e=String(t||"").trim();try{/^\d{4}$/.test(e)?sessionStorage.setItem(P,e):sessionStorage.removeItem(P)}catch(o){}return/^\d{4}$/.test(e)?e:""}function nt(){var t;return D()||G()||String(((t=document.getElementById("login-cohort-select"))==null?void 0:t.value)||"").trim()}function ot(t){const e=document.getElementById("login-graduate-cohort-panel"),o=document.getElementById("login-graduate-cohort-select");if(!e||!o)return;const a=H(),n=t!=="parent"&&a.length>0;if(e.hidden=!n,e.style.display=n?"":"none",e.setAttribute("aria-hidden",n?"false":"true"),!n){x("");return}const l=a.join("|");o.dataset.cohortYears!==l&&(o.innerHTML=a.map(r=>`<option value="${r}">${r}届 · 已毕业</option>`).join(""),o.dataset.cohortYears=l);const s=D();e.classList.toggle("is-selected",!!s&&a.includes(s)),s&&a.includes(s)&&(o.value=s)}function lt(){const t=document.getElementById("login-graduate-cohort-button"),e=document.getElementById("login-graduate-cohort-select"),o=document.getElementById("login-graduate-cohort-helper"),a=document.getElementById("login-cohort-select");a&&a.dataset.graduateResetBound!=="1"&&(a.dataset.graduateResetBound="1",a.addEventListener("change",()=>{var n;window.__REQUIRE_MANUAL_COHORT__=!1,W(a.value),x(""),(n=document.getElementById("login-graduate-cohort-panel"))==null||n.classList.remove("is-selected")})),!(!t||t.dataset.graduateBound==="1")&&(t.dataset.graduateBound="1",t.addEventListener("click",()=>{var s;const n=x((e==null?void 0:e.value)||"");(s=document.getElementById("login-graduate-cohort-panel"))==null||s.classList.toggle("is-selected",!!n),o&&(o.textContent=n?`已选择 ${n}届毕业生档案，登录后进入该届成绩。`:"请选择毕业届。");const l=document.getElementById("login-portal-helper");l&&n&&(l.textContent=`已选择 ${n}届毕业生成绩档案，请完成登录。`)}))}function it(t){var h;const e=(h=document.getElementById("login-pass"))==null?void 0:h.closest(".login-entry-field"),o=document.getElementById("login-form");if(!e||!o)return;let a=document.getElementById("login-cohort-group");a||(a=document.createElement("div"),a.id="login-cohort-group",a.className="login-clean-cohort",a.innerHTML=['<label for="login-cohort-select" class="login-clean-label">选择届别</label>','<div class="login-entry-field login-entry-field--select" data-login-field="cohort">','<span class="login-entry-prefix">Cohort</span>','<select id="login-cohort-select" data-login-cohort-select="1"></select>',"</div>"].join(""),e.insertAdjacentElement("afterend",a));const n=document.getElementById("login-cohort-select");if(!n)return;const l=N(),s=k(),r=G(),d=window.__REQUIRE_MANUAL_COHORT__===!0,p=n.dataset.cohortInitialized==="1"&&l.includes(n.value),u=d?"":l.includes(r)?r:p?n.value:s,v=['<option value="">请选择届别</option>'].concat(l.map(m=>`<option value="${m}">${m}届</option>`)).join("");n.dataset.cohortYears!==l.join("|")&&(n.innerHTML=v,n.dataset.cohortYears=l.join("|")),n.value=u,u&&W(u),n.dataset.cohortInitialized="1",a.style.display=t==="parent"?"none":"",a.setAttribute("aria-hidden",t==="parent"?"true":"false"),lt(),ot(t)}function st(){document.querySelectorAll(["#role-student",'label[for="role-student"]',"#form-student",'[data-portal="student"]','[data-login-open="student"]'].join(",")).forEach(t=>t.remove())}function rt(t){const e=document.querySelectorAll('#btn-role-school, [data-portal="school"], [data-login-open="school"]'),o=document.querySelectorAll('#btn-role-parent, [data-portal="parent"], [data-login-open="parent"]');e.forEach(a=>{a.textContent!=="学校端"&&(a.textContent="学校端"),a.classList.toggle("active",t==="school"),a.setAttribute("aria-pressed",t==="school"?"true":"false")}),o.forEach(a=>{a.textContent!=="家长端登录"&&(a.textContent="家长端登录"),a.classList.toggle("active",t==="parent"),a.setAttribute("aria-pressed",t==="parent"?"true":"false")})}function w(){const t=document.getElementById("login-overlay");if(!t)return;const e=q(),o=O[e]||O.school;st(),rt(e),t.dataset.loginPortal=e,document.querySelectorAll("#login-overlay .advanced-input-group i, #login-overlay .login-field-icon").forEach(p=>p.remove());const a=document.getElementById("login-user"),n=document.getElementById("login-class"),l=document.getElementById("login-pass");A(a,"user",o.userPrefix),A(n,"class","Class"),A(l,"password",o.passPrefix),it(e),a&&(a.placeholder=o.userPlaceholder),n&&(n.placeholder="请输入学生班级，如 701"),l&&(l.placeholder="输入密码");const s=document.getElementById("login-class-group");if(s){const p=e==="parent";s.style.display=p?"block":"none",s.setAttribute("aria-hidden",p?"false":"true")}const r=document.getElementById("login-submit-button");r&&r.dataset.bootBusy!=="1"&&r.textContent!==o.submit&&(r.textContent=o.submit);const d=document.getElementById("login-portal-helper");d&&!String(d.textContent||"").includes("正在")&&d.textContent!==o.helper&&(d.textContent=o.helper)}const ct={getLoginPortal:function(){return window.Auth&&typeof window.Auth.getLoginPortal=="function"?window.Auth.getLoginPortal():q()},ensureSystemIntroModal:function(){return window.Auth&&typeof window.Auth.ensureSystemIntroModal=="function"?window.Auth.ensureSystemIntroModal():null},rebuildInstagramLoginShell:function(){const t=document.getElementById("login-overlay");if(!t)return null;if(t.dataset.igRebuilt==="true")return t;const e=t.dataset.loginPortal==="parent"?"parent":"school";t.dataset.loginPortal=e;const o=`
