(()=>{if(typeof window=="undefined"||window.__MODULE_HELP_RUNTIME_PATCHED__)return;const o={upload:{title:"📁 数据上传与设置·使用说明",fit:"用于<strong>导入并规范化成绩数据</strong>，为后续所有分析提供可靠数据基础。",when:"每次考试结束后、首次使用或更换数据来源时使用。",use:`<ul>
                   ...
                   * 6-8年级权重：60/70/70；9年级权重：50/80/50。`},"high-score":{title:"🌟 9年级高分段核算·算法说明",fit:"用于<strong>尖子生培养</strong>与拔尖人才监测。",when:"中考备考阶段或重点关注拔尖学生结构时使用。",use:"仅针对 9 年级中考备考。统计总分 ≥ 490分 (可配置) 的尖子生情况。",calc:`<div class="formula-box">得分 = (本校高分率 ÷ 全镇最高高分率) × 70</div>
                   ...
                   附加分 = (超额人数 ÷ 全镇最大超额数) × 5
                   </div>`},summary:{title:"📑 综合分析报告·计算方式",fit:"用于<strong>汇总全模块成绩</strong>形成总排名报告。",when:"需要一键出具综合汇报或向上级汇报时使用。",use:"点击“生成总排名”汇总所有模块得分。",calc:'<div class="formula-box">非9年级：总榜得分 = 两率一分得分 + 后1/3得分<br>9年级：总榜得分 = 两率一分得分 + 后1/3得分 + 指标生得分 + 高分段赋分 + 高中上线率赋分(50)</div>'},teacher:{title:"👩‍🏫 教师教学质量画像·评价模型",fit:"用于<strong>教师教学成效</strong>与班级贡献度分析。",when:"完成教师任课配置后，进行教学质量复盘时使用。",use:"查看每位老师的实绩。需先在数据中心配置【教师任课】。",calc:`<strong>综合绩效分 (默认模型)：</strong><br>
