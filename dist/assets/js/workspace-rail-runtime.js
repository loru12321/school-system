                ...
                ${a.map((d,H)=>`
                    <button type="button"
                        ...
                        <span class="module-subnav-dock__label">
                            <strong>${String(d.text||`子模块 ${H+1}`)}</strong>
                            <span>${String(d.hint||"点击切换")}</span>
                        </span>
                    </button>
                `).join("")}
            </div>
        `}function u(){g||(g=window.requestAnimationFrame(()=>{g=0,D()}))}function U(){w||(w=!0,document.addEventListener("click",()=>window.setTimeout(u,30),!0),window.addEventListener("hashchange",u))}function j(e){var l;if(!e||e.dataset.analysisRailReady==="true")return;const t=e.querySelector(".analysis-side-nav"),n=e.querySelector(".content-area");if(!t||!n)return;const o=(((l=t.querySelector(".side-nav-title"))==null?void 0:l.textContent)||"功能导航").trim(),s=document.createElement("div");s.className="analysis-side-toolbar";const a=E("analysis-side-toggle","收起"+o,"ti-chevrons-left");a.addEventListener("click",function(){y(e,!0)}),s.appendChild(a),t.prepend(s);const i=n.querySelector(".analysis-content-stack")||n,c=document.createElement("div");c.className="analysis-side-reveal";const r=E("analysis-side-reveal-btn","展开"+o,"ti-chevrons-right");r.addEventListener("click",function(){y(e,!1)}),c.appendChild(r),i.prepend(c),e.__analysisSideNav=t,e.__analysisCollapseButton=a,e.__analysisRevealButton=r,e.__analysisRailTitle=o,e.dataset.analysisRailReady="true",C(e)}function k(){Array.from(document.querySelectorAll(".analysis-results-layout")).forEach(t=>{j(t),y(t,$(t),{rememberState:!1})}),u()}function T(){v||(v=window.requestAnimationFrame(function(){v=0,k()}))}function z(){S(),k()}window.toggleAppSidebar=M,window.setAppSidebarCollapsed=h,window.refreshAnalysisSideRails=T,window.refreshModuleSubnavDock=u,document.addEventListener("DOMContentLoaded",function(){U(),S(),k(),D(),new MutationObserver(T).observe(document.body,{childList:!0,subtree:!0})}),window.addEventListener("resize",z)})();
