(()=>{if(typeof window=="undefined"||window.__REPORT_RENDER_RUNTIME_PATCHED__)return;const F=window.CompareSessionState||null,nt=window.ReportSessionState||null,wt=typeof window.readCloudStudentCompareContextState=="function"?window.readCloudStudentCompareContextState:(()=>F&&typeof F.getCloudStudentCompareContext=="function"&&F.getCloudStudentCompareContext()||null),Ft=typeof window.readCurrentReportStudentState=="function"?window.readCurrentReportStudentState:(()=>nt&&typeof nt.getCurrentReportStudent=="function"?nt.getCurrentReportStudent()||null:window.CURRENT_REPORT_STUDENT&&typeof window.CURRENT_REPORT_STUDENT=="object"?window.CURRENT_REPORT_STUDENT:null),$t=typeof window.readDuplicateCompareExamsState=="function"?window.readDuplicateCompareExamsState:(()=>F&&typeof F.getDuplicateCompareExams=="function"?F.getDuplicateCompareExams()||[]:[]);function lt(a){return typeof getCloudCompareHint=="function"?getCloudCompareHint(a):isCloudContextMatchStudent(a)||isCloudContextLikelyCurrentTarget(a)?wt():null}function m(a,u,I="score"){if(u==null||u==="-"||u==="")return"";const l=parseFloat(a),O=parseFloat(u);if(isNaN(l)||isNaN(O))return"";const v=l-O;if(Math.abs(v)<.01)return'<span style="color:#94a3b8; font-size:11px; margin-left:4px; font-weight:normal;">(持平)</span>';let x="",w="",$="";I==="score"?v>0?(x="#15803d",$="#dcfce7",w="▲"):(x="#b91c1c",$="#fee2e2",w="▼"):v<0?(x="#15803d",$="#dcfce7",w="▲"):(x="#b91c1c",$="#fee2e2",w="▼");const o=Math.abs(v);return`<span style="display:inline-flex; align-items:center; background:${$}; color:${x}; padding:1px 6px; border-radius:10px; font-size:11px; font-weight:bold; margin-left:5px; vertical-align:middle;">
            ${w} ${I==="score"?o.toFixed(1):o}
        </span>`}function dt(a,u){var gt,ut,xt,bt,mt,ht,yt,vt;if(navigator.userAgent.toLowerCase().includes("android")&&window.innerWidth<=768&&!window.Chart)return console.warn("⚠️ Android Canvas 异常，强制切换 PC 模式"),dt(a,"PC");const v=window.innerWidth<=768;if(!(u==="A4"||u==="PC"||u==="FULL")&&v||u==="IG"){const t=ct(a);return setTimeout(()=>{typeof renderIGCharts=="function"&&renderIGCharts(a)},50),t}const w=RAW_DATA.length,$=new Date().toLocaleDateString(),o=getComparisonStudentView(a,RAW_DATA),b=lt(o),c=(b==null?void 0:b.previousRecord)||findPreviousRecord(o),R=typeof getStudentExamHistory=="function"?getStudentExamHistory(o):[],J=getEffectiveCurrentExamId(),U=R.filter(t=>{const r=t.examFullKey||t.examId;return!J||!isExamKeyEquivalentForCompare(r,J)&&!isExamKeyEquivalentForCompare(t.examId,J)}).slice(-1)[0]||null,T=U?U.student||U:null,f=T&&T.scores?T:c,_=((gt=f==null?void 0:f.ranks)==null?void 0:gt.total)||{},pt=t=>typeof t=="number"&&Number.isFinite(t)?t:t&&typeof t=="object"&&typeof t.score=="number"&&Number.isFinite(t.score)?t.score:"-",g=t=>String(t!=null?t:"").replace(/&/g,"&amp;").replace(/"/g,"&quot;").replace(/</g,"&lt;").replace(/>/g,"&gt;"),i=(t,r,s="")=>`<td data-label="${g(t)}"${s?` style="${s}"`:""}>${r}</td>`,A=t=>{var r,s,e,d,N,H;return{class:(s=(r=t==null?void 0:t.class)!=null?r:t==null?void 0:t.rankClass)!=null?s:"-",school:(d=(e=t==null?void 0:t.school)!=null?e:t==null?void 0:t.rankSchool)!=null?d:"-",township:(H=(N=t==null?void 0:t.township)!=null?N:t==null?void 0:t.rankTown)!=null?H:"-"}},Y=t=>{var e;const r=String((e=t==null?void 0:t.class)!=null?e:"").trim(),s=typeof normalizeClass=="function"?normalizeClass(r):r;return!s||s==="-"?!1:!/^(?:无|未分班|无班级|暂无|undefined|null|nan)$/i.test(s)},Q=t=>{if(typeof isCountyDirectStudentForRank=="function")return isCountyDirectStudentForRank(t);const r=String((t==null?void 0:t.school)||"").trim();if(!r||typeof getCountyDirectSchoolNames!="function"||typeof getTownshipManagedSchoolNames!="function")return!1;const s=Array.from(new Set([...Object.keys(SCHOOLS||{}),...(RAW_DATA||[]).map(d=>d==null?void 0:d.school)].map(d=>String(d||"").trim()).filter(Boolean)));return getTownshipManagedSchoolNames(s).length?getCountyDirectSchoolNames(s).some(d=>d===r||typeof areSchoolNamesEquivalent=="function"&&areSchoolNamesEquivalent(d,r)||typeof areSchoolNamesMatched=="function"&&areSchoolNamesMatched(d,r,!0)):!1},z=(t,r=!0)=>r?t==null||t===""?"-":t:"-",G=o&&typeof o=="object"&&o.scores&&typeof o.scores=="object"?o.scores:{},M=Y(o),D=!Q(o),X=z(safeGet(o,"ranks.total.township","-"),D),L=z((xt=(ut=_.township)!=null?ut:c==null?void 0:c.townRank)!=null?xt:"-",D),K=z(safeGet(o,"ranks.total.class","-"),M),n=z((mt=(bt=_.class)!=null?bt:c==null?void 0:c.classRank)!=null?mt:"-",M),p=safeGet(o,"ranks.total.school","-"),E=(yt=(ht=_.school)!=null?ht:c==null?void 0:c.schoolRank)!=null?yt:"-",h=getStudentCountyRankValue(o,"total"),tt=(vt=_.county)!=null?vt:"-",C=Object.keys(SCHOOLS).length<=1,y=C?"display:none !important;":"";let P="";if(CONFIG.name==="9年级"){let t=0,r=0;["语文","数学","英语","物理","化学"].forEach(s=>{G[s]!==void 0&&(t+=G[s],r++)}),r>0&&(P+=`<tr style="background:rgba(248,250,252,0.5);">
                    ...
                    ${i("全镇排名","-",y)}
                    ${i("全县排名","-",y)}
                </tr>`)}const j=getComparisonTotalSubjects(),S=getComparisonTotalValue(o,j),V=CONFIG.name==="9年级"&&j.length?"五科总分":CONFIG.label,rt=f?recalcPrevTotal(f):"-",at=m(S,rt,"score"),W=M?m(K,n,"rank"):"",B=m(p,E,"rank"),k=D?m(X,L,"rank"):"",Z=m(h,tt,"rank");P+=`<tr style="background:rgba(239,246,255,0.7); backdrop-filter:blur(4px); border-bottom:2px solid #fff;">
            ${i("科目",`🏆 ${V}`,"font-weight:bold; color:#1e3a8a;")}
            ${i("成绩（对比）",`${Number.isFinite(S)?S.toFixed(2):"-"} ${at}`,"font-weight:800; font-size:16px; color:#1e40af;")}
            ${i("班级排名",`${K} ${W}`,"font-weight:bold; color:#334155;")}
            ${i("校级排名",`${p} ${B}`,"font-weight:bold; color:#334155;")}
            ${i("全镇排名",`${X} ${k}`,`${y} font-weight:bold; color:#334155;`)}
            ${i("全县排名",`${h} ${Z}`,`${y} font-weight:bold; color:#334155;`)}
        </tr>`,[...new Set(SUBJECTS)].forEach(t=>{if(G[t]!==void 0){const r=f&&f.scores?pt(f.scores[t]):"-",s=m(G[t],r,"score");let e=A(f&&f.ranks?f.ranks[t]:null);e.class==="-"&&e.school==="-"&&e.township==="-"&&c&&c.ranks&&c.ranks[t]&&(e=A(c.ranks[t]));const d=safeGet(o,`ranks.${t}.school`,"-"),N=m(d,e.school||"-","rank"),H=z(safeGet(o,`ranks.${t}.township`,"-"),D),st=D?m(H,e.township||"-","rank"):"",q=getStudentCountyRankValue(o,t),ot=m(q,e.county||"-","rank");P+=`<tr style="transition:0.2s;" onmouseover="this.style.background='rgba(241,245,249,0.5)'" onmouseout="this.style.background='transparent'">
                    ${i("科目",t,"font-weight:600; color:#475569;")}
                    ${i("成绩（对比）",`${G[t]} ${s}`,"font-weight:bold; color:#334155;")}
                    ${i("总分班排","-","color:#cbd5e1;")}
                    ${i("校级排名",`${d} <span style="font-size:0.9em;">${N}</span>`,"color:#64748b;")}
                    ${i("全镇排名",`${H} <span style="font-size:0.9em;">${st}</span>`,`color:#64748b; ${y}`)}
                    ${i("全县排名",`${q} <span style="font-size:0.9em;">${ot}</span>`,`color:#64748b; ${y}`)}
                </tr>`}});const Ct=`
            <style>
        ...
        `,St=buildChartNarrative(o),et=buildStudentInsightModel(o,R),kt=renderStudentInsightOverview(et),Rt=renderStudentActionPlan(et),Tt=renderStudentSubjectBoard(et),zt=renderStudentRealityNote(et),Et=b?`
        <div class="fluent-card" style="padding:10px 14px; margin-bottom:12px; background:#eef2ff; border:1px solid #c7d2fe; color:#3730a3;">
            <div style="display:flex; align-items:center; gap:8px; font-size:12px; flex-wrap:wrap;">
                ...
                <span>当前对比：${b.prevExamId||"上次"} → ${b.latestExamId||"本次"}</span>
                <span style="color:#6366f1;">来源：${b.title||"云端记录"}</span>
            </div>
        </div>`:"",jt=$t().length>0?`
        ...
        <div class="fluent-card" style="padding:0; overflow:hidden;">
            <table class="fluent-table" id="tb-query">
                <thead><tr><th style="text-align:left; padding-left:20px;">科目</th><th>成绩 (对比)</th><th>总分班排</th><th>校排</th><th style="${y}">全镇排名</th><th style="${y}">全县排名</th></tr></thead>
                <tbody>${P}</tbody>
            </table>
        </div>`,it=R;let ft="";if(it.length>1){let t="",r=`<th style="text-align:left; padding-left:20px;">考试名称</th><th>${V}</th><th>校排</th>`;C||(r+="<th>镇排</th>");for(let s=it.length-1;s>=0;s--){const e=it[s],d=e.examFullKey||e.examId,N=getEffectiveCurrentExamId(),H=!!N&&(isExamKeyEquivalentForCompare(d,N)||isExamKeyEquivalentForCompare(e.examId,N)),st=H?"background:rgba(239,246,255,0.7); font-weight:bold;":"",q=e.student||e,ot=getComparisonTotalValue(q,j),Ht=Number.isFinite(ot)?ot.toFixed(1):"-",At=safeGet(q,"ranks.total.school",e.rankSchool||"-"),Dt=safeGet(q,"ranks.total.township",e.rankTown||"-");t+=`<tr style="${st}">
                ${i("考试名称",`${H?"⭐ ":""}${e.examLabel||e.examId||e.examFullKey||"-"}`,"text-align:left; padding-left:20px; color:#475569;")}
                ${i(V,Ht,"color:#2563eb;")}
                ${i("校级排名",At,"color:#64748b;")}
                ${C?"":i("全镇排名",Dt,"color:#64748b;")}
            </tr>`}ft=`
        ...
        ${St}
        <div style="text-align:center; font-size:11px; color:#cbd5e1; margin-top:20px;">系统自动生成 · 仅供家校沟通参考</div>`}function ct(a){var L,K;const u=new Date().toLocaleDateString(),I=RAW_DATA.length,l=getComparisonStudentView(a,RAW_DATA),O=getComparisonTotalSubjects(),v=getComparisonTotalValue(l,O),x=!isCountyDirectStudent(l),w=x?safeGet(l,"ranks.total.township","-"):safeGet(l,"ranks.total.school","-"),$=(l==null?void 0:l.school)&&((K=(L=SCHOOLS==null?void 0:SCHOOLS[l.school])==null?void 0:L.students)==null?void 0:K.length)||I||1,b=typeof w=="number"?((1-w/(x?I||1:$))*100).toFixed(0):"-",c=a.name.charAt(0),R=lt(l),U=Object.keys(SCHOOLS).length<=1?"全校":x?"全镇":"本校";let T="";b>=90?T="🌟 卓越之星":b>=75?T="🔥 进步飞速":T="📚 持续努力";let f="";O.forEach(n=>{if(l.scores[n]!==void 0){const p=l.scores[n],E=safeGet(l,`ranks.${n}.school`,"-"),h=x?safeGet(l,`ranks.${n}.township`,"-"):"-",tt=getStudentCountyRankValue(l,n),C=[`级#${E}`];x&&C.push(`镇#${h}`),C.push(`县#${tt}`),f+=`
        ...
        `,g=(()=>{let n=[],p=[],E=[],h=[];getComparisonTotalSubjects().forEach(j=>{if(a.scores[j]!==void 0){const S=RAW_DATA.map(k=>k.scores[j]).filter(k=>typeof k=="number");if(S.length<2)return;const V=S.reduce((k,Z)=>k+Z,0)/S.length,rt=S.reduce((k,Z)=>k+Math.pow(Z-V,2),0)/S.length,at=Math.sqrt(rt)||1,W=(a.scores[j]-V)/at;h.push(W);const B=`${j}`;W>=.8?n.push(B):W<=-.8?p.push(B):E.push(B)}});const C=h.length?Math.max(...h):0,y=h.length?Math.min(...h):0,P=C-y;return{strong:n,weak:p,mid:E,range:P}})(),A=(n=>n>=2.5?{tag:"⚠️ 严重偏科",color:"#b91c1c",bg:"#fee2e2",text:"不同学科成绩差异极大，存在明显优势科目与薄弱科目，需要针对性调整学习重心，补齐短板。"}:n>=1.2?{tag:"⚖️ 相对均衡",color:"#0369a1",bg:"#e0f2fe",text:"各学科成绩整体较为均衡，个别学科略有波动，保持稳定发挥是关键。"}:{tag:"🌟 结构优秀",color:"#15803d",bg:"#dcfce7",text:"各学科发展极其均衡，无明显短板，心理素质稳定，是冲刺更高目标的理想状态。"})(g.range),Y=`
        ...
        `,Q=(n,p)=>!n||n.length===0?`<div style="font-size:12px; color:#ccc; padding:5px;">${p}</div>`:n.map(E=>`<span style="display:inline-block; background:#f1f5f9; color:#334155; font-size:12px; padding:4px 10px; border-radius:4px; margin:0 5px 5px 0;">${E}</span>`).join(""),z=`
        ...
        `,D=`
            <div class="insta-visual-area">
                <div style="width:100%; height:100%; display:flex; flex-direction:column; justify-content:center; align-items:center; background:linear-gradient(135deg, #833ab4, #fd1d1d, #fcb045); border-radius:8px; color:white; padding:40px 0;">
                    ...
                    <div style="margin-top:20px; font-size:12px; opacity:0.8;">击败了${U} ${b}% 的考生</div>
                </div>
            </div>
        `,X=R?`
            <div style="margin:12px 14px 0 14px; background:#eef2ff; border:1px solid #c7d2fe; border-radius:8px; padding:10px 12px;">
                <div style="font-size:12px; color:#3730a3; line-height:1.5;">
                    ...
                    当前对比：${R.prevExamId||"上次"} → ${R.latestExamId||"本次"}
                </div>
                <div style="font-size:11px; color:#6366f1; margin-top:4px;">来源：${R.title||"云端记录"}</div>
            </div>
        `:"";return`
        ...
        `}Object.assign(window,{getTrendBadge:m,renderSingleReportCardHTML:dt,renderInstagramCard:ct}),window.__REPORT_RENDER_RUNTIME_PATCHED__=!0})();
