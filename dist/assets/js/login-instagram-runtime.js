(function(){function D(){return`
        ...
        `}function F(){if(!window.gsap)return;const a=document.getElementById("login-hero");a&&window.gsap.fromTo(a.children,{autoAlpha:0,y:20},{autoAlpha:1,y:0,duration:.8,stagger:.1,ease:"power3.out"});const n=document.querySelectorAll(".login-portal-card");n.length&&window.gsap.fromTo(n,{autoAlpha:0,scale:.95,y:15},{autoAlpha:1,scale:1,y:0,duration:.6,stagger:.1,ease:"back.out(1.7)",delay:.4})}function K(a,n){const e=n==="parent"?"parent":"school",t=e==="parent"?{badge:"家长成长入口",authTitle:"登录入口",userLabel:"学生姓名",userPlaceholder:"请输入学生姓名",userHelper:"建议使用学生姓名登录，并完整填写班级信息。",classNote:"(家长端必填，如 701)",classPlaceholder:"请输入学生班级，如 701",helper:"当前为家长端，验证后进入成长报告与成绩视图。",submit:"进入家长端",stageKicker:"Family Growth Portal",stageTitle:'<span class="login-stage-title-line">查看成长报告</span><span class="login-stage-title-line login-stage-title-line--accent">更轻、更清楚、更直接</span>',stageCopy:"为教育管理者与家长提供高效、直观的统一入口体验。",stageMeta:[{icon:"ti ti-heart-handshake",text:"成长报告 / 成绩查询 / 家校提醒"},{icon:"ti ti-devices",text:"手机、安卓与桌面端共用同一套入口"},{icon:"ti ti-sparkles",text:"当前稳定版 v1.0 · 2026-04-08"}],launchKicker:"登录窗口",launchCopy:"先选择家长端，再打开唯一登录窗口；表单、说明和下载都各归其位。",launchNote:"应用下载会打开下载中心，系统介绍会说明角色权限、流程和成绩规则。",stageFeatureTitle:"家长端聚焦成绩、报告与提醒",stageFeatureCopy:"首页只留下最重要的入口和价值点，避免像旧版那样把所有信息都堆在首屏。",modalChip:"家长端登录窗口",modalTitle:"进入家长端",modalCopy:"输入学生姓名、班级与密码后，直接查看成长报告、成绩与家校提醒。",navButton:"打开家长端"}:{badge:"学校工作台",authTitle:"登录入口",userLabel:"账号 / 姓名",userPlaceholder:"管理员账号 / 教师姓名",userHelper:"支持管理员、教务、年级、班主任与教师账号登录。",classNote:"(学校端无需填写)",classPlaceholder:"学校端无需填写",helper:"当前为学校端，验证通过后直达教学分析与数据维护。",submit:"进入学校工作台",stageKicker:"School Command Center",stageTitle:'<span class="login-stage-title-line">统一登录</span><span class="login-stage-title-line login-stage-title-line--accent">把学校端和家长端放在一张首页里</span>',stageCopy:"学校端与家长端深度集成，为多角色协作提供流畅的数字化基础。",stageMeta:[{icon:"ti ti-layout-dashboard",text:"教学分析 / 数据维护 / 学校工作台"},{icon:"ti ti-devices",text:"Web、Android 与 Desktop 共用入口逻辑"},{icon:"ti ti-sparkles",text:"当前稳定版 v1.0 · 2026-04-08"}],launchKicker:"登录窗口",launchCopy:"先选学校端或家长端，再在唯一登录窗口里完成验证，减少跳转和视觉噪音。",launchNote:"应用下载会打开双端下载中心，系统介绍会说明模块结构、角色权限和核心逻辑。",stageFeatureTitle:"登录、下载与系统说明各自独立",stageFeatureCopy:"首屏只负责建立品牌感和主入口，不再把所有解释文字都堆到同一块大面板里。",modalChip:"学校端登录窗口",modalTitle:"进入学校端",modalCopy:"输入账号与密码后，直接进入教学分析、数据维护与学校工作台。",navButton:"打开学校端"};Object.entries({school:{title:"学校端",desc:"教学分析、数据维护与学校管理入口。",meta:"Analysis / Data / Workspace",action:"打开学校端窗口"},parent:{title:"家长端",desc:"成长报告、成绩查询与家校提醒入口。",meta:"Report / Score / Reminder",action:"打开家长端窗口"}}).forEach(([l,i])=>{const o=document.querySelector(`.login-portal-card[data-portal="${l}"]`);if(!o)return;const M=o.querySelector(".login-portal-title"),P=o.querySelector(".login-portal-desc"),H=o.querySelector(".login-portal-meta"),q=o.querySelector(".login-portal-action");M&&(M.textContent=i.title),P&&(P.textContent=i.desc),H&&(H.textContent=i.meta),q&&(q.textContent=i.action)});const d=document.getElementById("login-portal-badge"),r=document.getElementById("login-user-label"),g=document.getElementById("login-user"),p=document.getElementById("login-user-helper"),c=document.getElementById("login-class-group"),u=document.getElementById("login-class"),h=document.getElementById("login-class-label-note"),m=document.getElementById("login-portal-helper"),y=document.getElementById("login-submit-button"),v=document.getElementById("login-stage-kicker"),b=document.getElementById("login-stage-title"),f=document.getElementById("login-stage-copy"),w=document.querySelector(".login-stage-meta"),k=document.getElementById("login-stage-featured-title"),C=document.getElementById("login-stage-featured-copy"),x=document.getElementById("login-modal-chip"),A=document.getElementById("login-modal-title"),E=document.getElementById("login-modal-copy"),S=document.querySelector(".login-auth-title"),I=document.querySelector(".login-portal-launch-head span"),L=document.querySelector(".login-portal-launch-head p"),B=document.querySelector(".login-portal-note"),T=document.querySelector(".login-stage-nav-login");d&&(d.textContent=t.badge),r&&(r.textContent=t.userLabel),g&&(g.placeholder=t.userPlaceholder),p&&(p.textContent=t.userHelper),h&&(h.textContent=t.classNote),u&&(u.placeholder=t.classPlaceholder),c&&(c.style.display=e==="parent"?"block":"none",c.setAttribute("aria-hidden",e==="parent"?"false":"true")),m&&(m.textContent=t.helper),y&&(y.textContent=t.submit),v&&(v.textContent=t.stageKicker),b&&(b.innerHTML=t.stageTitle),f&&(f.textContent=t.stageCopy),w&&(w.innerHTML=(t.stageMeta||[]).map(l=>`<span><i class="${l.icon}"></i> ${l.text}</span>`).join("")),k&&(k.textContent=t.stageFeatureTitle),C&&(C.textContent=t.stageFeatureCopy),S&&(S.textContent=t.authTitle),I&&(I.textContent=t.launchKicker),L&&(L.textContent=t.launchCopy),B&&(B.textContent=t.launchNote),x&&(x.textContent=t.modalChip),A&&(A.textContent=t.modalTitle),E&&(E.textContent=t.modalCopy),T&&(T.textContent=t.navButton),a&&typeof a.renderSystemIntroModal=="function"&&a.renderSystemIntroModal(e),F()}function s(){return!window.Auth||typeof window.Auth.getLoginShellHtml!="function"?!1:(window.Auth.getLoginShellHtml=D,window.Auth.applyLoginShellCopy=K,window.Auth.__bootLoginShell="instagram",!0)}s()||document.addEventListener("DOMContentLoaded",()=>{if(s())return;const a=window.setInterval(()=>{s()&&window.clearInterval(a)},80);window.setTimeout(()=>window.clearInterval(a),4e3)},{once:!0})})();
