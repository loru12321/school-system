(()=>{if(typeof window=="undefined"||window.__FRESHMAN_EXAM_RUNTIME_PATCHED__)return;let F=[],b=typeof window.readFbClassesState=="function"?window.readFbClassesState():Array.isArray(window.FB_CLASSES)?window.FB_CLASSES:[],E=-1,P={},M=[],S=[],$=[],J=null;function Ct(){const t=typeof window.readFbClassesState=="function"?window.readFbClassesState():Array.isArray(window.FB_CLASSES)?window.FB_CLASSES:[];return Array.isArray(t)&&(b=t),window.FB_CLASSES=b,b}function rt(t){const e=typeof window.setFbClassesState=="function"?window.setFbClassesState(t):Array.isArray(t)?t:[];return b=Array.isArray(e)?e:[],window.FB_CLASSES=b,b}function it(){return window.CONFIG||{name:"学校"}}function lt(t){const e=t.files[0];if(!e)return;const a=new FileReader;a.onload=function(s){try{const o=new Uint8Array(s.target.result),n=XLSX.read(o,{type:"array"}),r=XLSX.utils.sheet_to_json(n.Sheets[n.SheetNames[0]]);if(!r.length)throw new Error("Excel没有数据");F=r.map((i,c)=>{const f=String(i.备注||i.说明||""),g=f.match(/(?:和|与|跟)([\u4e00-\u9fa5\w]+)(?:同班|一起|一班)/),m=f.match(/(?:和|与|跟)([\u4e00-\u9fa5\w]+)(?:分开|不同班|不在一起)/);return{_id:c,name:i.姓名||"未知",gender:i.性别==="男"||i.Gender==="M"?"M":"F",score:parseFloat(i.总分||i.语数英||0),height:parseFloat(i.身高||160),vision:parseFloat(i.视力||i.左眼||5),isDiff:String(i.难管||"").includes("是")||f.includes("难管")||f.includes("调皮"),remarks:f,constraints:{same:g?[g[1]]:[],diff:m?[m[1]]:[]},classIdx:-1}}),alert(`✅ 导入成功！共 ${F.length} 人。`),document.getElementById("fb-results-area").classList.add("hidden")}catch(o){alert("读取失败："+o.message)}},a.readAsArrayBuffer(e)}function Mt(t){const e=q(t),a=Math.floor(t.length/2),s=t.slice(0,a),o=t.slice(t.length%2===0?a:a+1),n=q(s),r=q(o);return{q1:n,q2:e,q3:r}}function q(t){const e=Math.floor(t.length/2);return t.length%2!==0?t[e]:(t[e-1]+t[e])/2}function dt(t){const e=t.length;if(e===0)return 0;const a=t.reduce((o,n)=>o+n,0)/e,s=t.reduce((o,n)=>o+Math.pow(n-a,2),0)/e;return Math.sqrt(s)}function ct(){if(!F.length)return alert("请先导入数据");const t=parseInt(document.getElementById("fb_cls_num").value)||6,e=document.getElementById("fb_algorithm").value,a=document.querySelector('button[onclick="FB_runDivision()"]');a.innerHTML="⏳ 正在运算多套方案...",a.disabled=!0,setTimeout(()=>{$=[];const s=e==="snake"?1:3;for(let r=0;r<s;r++){const i=j(t,e),c=i.map(m=>m.stats.avg),f=Math.max(...c)-Math.min(...c),g=dt(c);$.push({id:r,name:s===1?"标准方案":`方案 ${String.fromCharCode(65+r)}`,data:i,range:f,sd:g,desc:`均分极差 ${f.toFixed(2)}`})}a.innerHTML="🚀 开始智能分班",a.disabled=!1,z();const o=[...$].sort((r,i)=>r.range-i.range)[0];U(o.id),document.getElementById("fb-results-area").classList.remove("hidden");const n=document.getElementById("fb-scheme-panel");s>1?n&&n.classList.remove("hidden"):n&&n.classList.add("hidden")},100)}function j(t,e){let a=Array.from({length:t},(o,n)=>({id:n,name:n+1+"班",students:[],stats:{}})),s=JSON.parse(JSON.stringify(F));if(s.sort((o,n)=>n.score-o.score),e==="snake")s.forEach((o,n)=>{const i=Math.floor(n/t)%2===0?n%t:t-1-n%t;a[i].students.push(o),o.classIdx=i});else{s.forEach((r,i)=>{const c=Math.floor(i/t)%2===0?i%t:t-1-i%t;a[c].students.push(r),r.classIdx=c});const o=8e3,n=s.reduce((r,i)=>r+i.score,0)/s.length;for(let r=0;r<o;r++){const i=Math.floor(Math.random()*t),c=Math.floor(Math.random()*t);if(i===c)continue;const f=a[i],g=a[c];if(!f.students.length||!g.students.length)continue;const m=Math.floor(Math.random()*f.students.length),d=Math.floor(Math.random()*g.students.length),p=f.students[m],h=g.students[d],u=T(f,n)+T(g,n);f.students[m]=h,h.classIdx=i,g.students[d]=p,p.classIdx=c;const y=T(f,n)+T(g,n);let w=!1;(D(p,g.students)||D(h,f.students))&&(w=!0),(w||y>u)&&(f.students[m]=p,p.classIdx=i,g.students[d]=h,h.classIdx=c)}}return a.forEach(o=>{const n=o.students.length,r=o.students.reduce((i,c)=>i+c.score,0);o.stats.avg=n?r/n:0,o.stats.male=o.students.filter(i=>i.gender==="M").length,o.stats.count=n}),a}function z(){const t=document.getElementById("fb-scheme-cards");if(!t)return;t.innerHTML="";const e=$.length?Math.min(...$.map(a=>a.range)):1/0;$.forEach(a=>{const s=a.range<=e,o=s?"border:2px solid #16a34a; background:#fff;":"border:1px solid #ddd; background:#fff;",n=a.data.map(i=>i.stats.male),r=Math.max(...n)-Math.min(...n);t.innerHTML+=`
            ...
            `})}function U(t){const e=$.find(s=>s.id===t);if(!e)return;rt(e.data),P={},b.forEach(s=>P[s.name]=s.students),V();const a=document.getElementById("fb-scheme-cards");if(a){const s=a.children;Array.from(s).forEach(o=>{String(o.dataset.schemeId)===String(e.id)?(o.style.borderColor="#16a34a",o.style.boxShadow="0 0 0 3px rgba(22, 163, 74, 0.2)"):(o.style.borderColor="#ddd",o.style.boxShadow="none")})}}function T(t,e){const a=t.students.length;if(a===0)return 1e4;const s=t.students.reduce((i,c)=>i+c.score,0)/a,o=t.students.filter(i=>i.gender==="M").length,n=t.students.filter(i=>i.isDiff||i._isDiff).length;let r=Math.pow(s-e,2)*100;return r+=Math.pow(o/a-.5,2)*5e3,document.getElementById("fb_rule_diff").value==="spread"&&(r+=Math.pow(n,2)*500),r}function D(t,e){if(!t.constraints)return!1;for(let a of t.constraints.diff)if(e.find(s=>s.name===a))return!0;return!1}function V(){document.getElementById("fb-results-area").classList.remove("hidden");const t=document.getElementById("fb_class_container");t.innerHTML="";let e=[],a=0,s=0,o=0;b.forEach(d=>{const p=d.students.length,h=d.students.reduce((x,k)=>x+k.score,0),u=p?h/p:0,y=d.students.filter(x=>x.gender==="M").length,w=d.students.filter(x=>x.isDiff||x._isDiff).length;e.push(u),a+=y,s+=p-y,o+=w,d.stats={avg:u,male:y,female:p-y,count:p};const B=w>3;t.innerHTML+=`<div class="fb-class-box ${B?"fb-warn-bg":""}" onclick="FB_openSeatMap(${d.id})"><div class="fb-c-head"><span style="font-weight:bold; font-size:16px;">${d.name}</span><span class="fb-tag fb-tag-red" style="${w>0?"":"display:none"}">难管: ${w}</span></div><div class="fb-c-body"><div>人数: <strong>${p}</strong></div><div>均分: <strong>${u.toFixed(1)}</strong></div><div>男生: ${y}</div><div>女生: ${p-y}</div><div style="grid-column:span 2; font-size:11px; color:#999; margin-top:5px;">点击进入座位编排 →</div></div></div>`});const n=e.length?Math.max(...e)-Math.min(...e):0,r=document.getElementById("fb_res_total"),i=document.getElementById("fb_res_male"),c=document.getElementById("fb_res_female"),f=document.getElementById("fb_res_diff"),g=document.getElementById("fb_res_diff_cnt");r&&(r.innerText=F.length),i&&(i.innerText=a),c&&(c.innerText=s),f&&(f.innerText=n.toFixed(2)),g&&(g.innerText=o);const m=document.getElementById("fb_res_eval");m&&(n<=1?m.innerHTML='<span style="color:green;font-weight:bold;">✅ 完美均衡</span>':n<=3?m.innerHTML='<span style="color:#d97706;font-weight:bold;">⚠️ 基本均衡</span>':m.innerHTML='<span style="color:red;font-weight:bold;">❌ 差异过大</span>'),G()}function G(){const t=document.getElementById("balanceChart"),e=document.getElementById("balanceTableContainer"),a=b.map(n=>n.name),s=b.map(n=>{const r=n.students.map(c=>c.score).sort((c,f)=>c-f),i=Mt(r);return{min:r[0],max:r[r.length-1],q1:i.q1,median:i.q2,q3:i.q3,avg:n.stats.avg,sd:dt(r)}});J&&J.destroy(),J=new Chart(t,{type:"bar",data:{labels:a,datasets:[{label:"平均分",data:s.map(n=>n.avg),type:"scatter",backgroundColor:"#2563eb",borderColor:"#2563eb",pointStyle:"rectRot",pointRadius:6},{label:"分数区间 (Min-Max)",data:s.map(n=>[n.min,n.max]),backgroundColor:"rgba(156, 163, 175, 0.2)",borderColor:"rgba(156, 163, 175, 0.5)",borderWidth:1,barPercentage:.1},{label:"核心分布 (Q1-Q3)",data:s.map(n=>[n.q1,n.q3]),backgroundColor:"rgba(37, 99, 235, 0.5)",borderColor:"#1e40af",borderWidth:1,barPercentage:.6}]},options:{responsive:!0,maintainAspectRatio:!1,plugins:{tooltip:{callbacks:{label:function(n){const r=s[n.dataIndex];if(n.dataset.type==="scatter")return`平均分: ${r.avg.toFixed(2)}`;if(n.datasetIndex===1)return`范围: ${r.min} - ${r.max}`;if(n.datasetIndex===2)return`核心区间: ${r.q1} - ${r.q3}`}}},title:{display:!0,text:"班级分数结构对比 (箱线图)"}},scales:{y:{beginAtZero:!1,title:{display:!0,text:"分数"}}}}});let o='<table class="comparison-table" style="font-size:12px;"><thead><tr><th>班级</th><th>人数</th><th>平均分</th><th>标准差 (SD)</th><th>极差 (Max-Min)</th><th>前25%线 (Q3)</th><th>后25%线 (Q1)</th></tr></thead><tbody>';s.forEach((n,r)=>{o+=`<tr><td>${a[r]}</td><td>${b[r].students.length}</td><td>${n.avg.toFixed(2)}</td><td>${n.sd.toFixed(2)}</td><td>${(n.max-n.min).toFixed(1)}</td><td>${n.q3}</td><td>${n.q1}</td></tr>`}),e.innerHTML=o+"</tbody></table>"}const W={past:[],future:[],limit:20,record:function(){if(E===-1)return;const t=b[E],e=JSON.parse(JSON.stringify(t));this.past.push(e),this.past.length>this.limit&&this.past.shift(),this.future=[],this.updateUI()},undo:function(){if(this.past.length===0)return;const t=JSON.parse(JSON.stringify(b[E]));this.future.push(t);const e=this.past.pop();b[E]=e,this.refreshView("已撤销 ↩")},redo:function(){if(this.future.length===0)return;const t=JSON.parse(JSON.stringify(b[E]));this.past.push(t);const e=this.future.pop();b[E]=e,this.refreshView("已重做 ↪")},refreshView:function(t){L(),this.updateUI(),UI.toast(t,"info")},updateUI:function(){const t=document.getElementById("btn_undo"),e=document.getElementById("btn_redo");t&&(t.disabled=this.past.length===0,t.className=this.past.length>0?"btn btn-primary":"btn btn-gray"),e&&(e.disabled=this.future.length===0,e.className=this.future.length>0?"btn btn-primary":"btn btn-gray")},reset:function(){this.past=[],this.future=[],this.updateUI()}};function ft(t){W.reset(),E=t;const e=b[t];document.getElementById("seat_class_title").innerText=e.name,document.getElementById("fb_seat_view").classList.remove("hidden"),document.getElementById("fb_seat_view").scrollIntoView({behavior:"smooth"}),updateConstraintWidgetsContext("fb"),e.seatLayout?L():K(),N()}const R=Object.create(null);function Lt(t){const e=String(t||"").trim(),a=[],s=o=>{const n=String(o||"").trim();!n||a.includes(n)||a.push(n)};if(s(e),window.location&&window.location.protocol==="file:"&&e.startsWith("./assets/")){const o=e.replace(/^\.\//,"");s(`./public/${o}`),s(`./dist/${o}`)}return a}function ut(t){return String(t||"").replace(/<\/script/gi,"<\\/script")}async function Q(t,e){if(R[t])return R[t];const a=Array.from(document.querySelectorAll(`script[data-standalone-lib="${t}"]`));for(const n of a){const r=String(n.textContent||"").trim();if(!n.src&&r)return R[t]=r,r}const s=[];a.forEach(n=>{const r=String(n.getAttribute("src")||n.src||"").trim();r&&!s.includes(r)&&s.push(r)}),Lt(e).forEach(n=>{s.includes(n)||s.push(n)});let o=null;for(const n of s)try{const r=await fetch(n,{cache:"force-cache"});if(!r.ok)throw new Error(`HTTP ${r.status}`);const i=await r.text();if(!i.trim())throw new Error("EMPTY_SOURCE");return R[t]=i,i}catch(r){o=r instanceof Error?r:new Error(String(r))}throw o||new Error(`${t} source unavailable`)}async function At(){const[t,e]=await Promise.all([Q("crypto-js","./assets/vendor/crypto-js/crypto-js.min.js"),Q("chart.js","./assets/vendor/chart.js/chart.umd.min.js")]);return{cryptoJsSource:ut(t),chartJsSource:ut(e)}}async function $t(){if(typeof CryptoJS!="undefined")return CryptoJS;if(typeof window.ensureCryptoJsVendorLoaded=="function")return window.ensureCryptoJsVendorLoaded();const t=await Q("crypto-js","./assets/vendor/crypto-js/crypto-js.min.js");if(window.eval(t),typeof CryptoJS=="undefined")throw new Error("CryptoJS runtime unavailable");return CryptoJS}async function Tt(){const t=document.getElementById("studentSchoolSelect").value;if(!t||t.includes("请选择"))return alert("请先选择一个学校，系统将生成该校的查分包。");if(typeof CryptoJS=="undefined")try{await $t()}catch(u){return console.error("[InquiryPackage] crypto-js load failed:",u),alert("❌ 导出失败：加密库未加载完成，请刷新页面后重试。")}const e=SCHOOLS[t].students;if(!e||e.length===0)return alert("该学校无数据");const a=Object.keys(SCHOOLS).length<=1,s={};SUBJECTS.forEach(u=>{const y=RAW_DATA.map(w=>w.scores[u]).filter(w=>typeof w=="number");if(y.length>0){const B=y.reduce((k,I)=>k+I,0)/y.length,x=y.reduce((k,I)=>k+Math.pow(I-B,2),0)/y.length;s[u]={avg:B,sd:Math.sqrt(x)}}else s[u]={avg:0,sd:1}});const o={};e.forEach(u=>{const y=(u.class+"_"+u.name).replace(/\s+/g,""),w={},B={labels:[],data:[]},x={labels:[],data:[]},k=typeof hasStudentClassRankScope=="function"?hasStudentClassRankScope(u):!0,I=typeof isCountyDirectStudentForRank=="function"?!isCountyDirectStudentForRank(u):!0;SUBJECTS.forEach(l=>{if(u.scores[l]!==void 0){w[l]=[u.scores[l],safeGet(u,`ranks.${l}.school`,"-"),I?safeGet(u,`ranks.${l}.township`,"-"):"-"];const v=RAW_DATA.map(H=>H.scores[l]).filter(H=>H!==void 0).sort((H,Ft)=>Ft-H),_=v.indexOf(u.scores[l])+1,C=v.length,A=((1-_/C)*100).toFixed(1);B.labels.push(l),B.data.push(A);const O=s[l];let It=0;O&&O.sd>0&&(It=(u.scores[l]-O.avg)/O.sd),x.labels.push(l),x.data.push(parseFloat(It.toFixed(2)))}});const X=`${u.school}_${u.class}_${u.name}`,ot=readBatchAICacheState()[X]||generateAIComment(u);o[y]={cls:u.class,name:u.name,s:w,t:u.total,tr:I?safeGet(u,"ranks.total.township","-"):"-",sr:safeGet(u,"ranks.total.school","-"),cr:k?safeGet(u,"ranks.total.class","-"):"-",showClassRank:k,showTownRank:I,rd:B,vd:x,cm:ot}});const n=prompt(`🔐 安全设置
...
1. 输入此密码
2. 输入准确的班级
3. 输入准确的姓名`,"123456");if(n===null)return;if(!n)return alert("❌ 必须设置密码才能生成安全查分包！");const r=JSON.stringify(o),i=CryptoJS.AES.encrypt(r,n).toString();let c=null;try{c=await At()}catch(u){return console.error("[InquiryPackage] standalone libs unavailable:",u),alert("❌ 导出失败：查分包依赖未准备好，请刷新页面后重试。")}const f=JSON.stringify(i).replace(/</g,"\\u003c"),g=it().name||"期中考试",m=new Date().toLocaleDateString(),d=`
<!DOCTYPE html>
<html lang="zh-CN">
<head>
<meta charset="UTF-8">
