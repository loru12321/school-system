...
async function getHistoryComparisonData(studentName, className, schoolName) {
    ...
    try {
        if (typeof sbClient !== 'undefined' && sbClient) {
            ...
            if (!result.error && result.data && result.data.length > 0) {
                var normalizedTargetName = normalizeCompareName(studentName);
                var normalizedTargetClass = String(className || '').trim();
                ...
                for (var i = 0; i < result.data.length; i++) {
                    ...
                    for (var schName in schools) {
                        var schData = schools[schName];
                        if (!schoolName || schName === schoolName || schName.includes(schoolName)) {
                            var students = schData.students || [];
                            for (var j = 0; j < students.length; j++) {
                                var s = students[j];
                                var sName = normalizeCompareName(s.name || '');
                                var sameName = sName === normalizedTargetName;
                                var sameClass = !normalizedTargetClass || isClassEquivalent(s.class || '', normalizedTargetClass);
                    ...
                    if (matchedStudent) {
                        var examName = item.key.replace('cohort::', '考试');
                        console.log('✅ 找到匹配学生:', matchedStudent.name, '考试:', examName);
                        history.push({
                            examId: examName,
                            total: matchedStudent.total,
                            rankClass: matchedStudent.ranks && matchedStudent.ranks.total ? matchedStudent.ranks.total.class : null,
                            rankSchool: matchedStudent.ranks && matchedStudent.ranks.total ? matchedStudent.ranks.total.school : null,
                            rankTown: matchedStudent.ranks && matchedStudent.ranks.total ? matchedStudent.ranks.total.township : null,
    ...
    if (typeof SCHOOLS !== 'undefined' && SCHOOLS && studentName) {
        for (var schKey in SCHOOLS) {
            var schData = SCHOOLS[schKey];
            var currentStu = null;
            if (schData.students) {
                for (var k = 0; k < schData.students.length; k++) {
                    var s = schData.students[k];
                    if (s.name === studentName && (!className || isClassEquivalent(s.class, className))) {
                        currentStu = s;
                        break;
                    }
                }
            ...
            if (currentStu) {
                ...
                history.push({
                    ...
                    rankClass: currentStu.ranks && currentStu.ranks.total ? currentStu.ranks.total.class : null,
                    rankSchool: currentStu.ranks && currentStu.ranks.total ? currentStu.ranks.total.school : null,
                    rankTown: currentStu.ranks && currentStu.ranks.total ? currentStu.ranks.total.township : null,
    ...
    history.sort(function(a, b) { return new Date(a.updatedAt) - new Date(b.updatedAt); });
...
function renderHistoryCharts(historyData, currentStudent) {
    var containerId = 'history-charts-container';
    ...
    if (!container) {
        ...
        var captureArea = document.getElementById('report-card-capture-area');
    ...
    var previous = historyData.length >= 2 ? historyData[historyData.length - 2] : null;
    
    var totalChange = previous ? calculateChange(latest.total, previous.total) : null;
    var rankSchoolChange = previous && latest.rankSchool && previous.rankSchool ? calculateChange(latest.rankSchool, previous.rankSchool) : null;
    var rankTownChange = previous && latest.rankTown && previous.rankTown ? calculateChange(latest.rankTown, previous.rankTown) : null;

    var html = '<div class="sec-head" style="padding-bottom:15px; border-bottom:1px solid #e2e8f0;"><h2 style="color:#1e293b; margin:0;"><i class="ti ti-timeline" style="color:#2563eb;"></i> 历史成绩记录与对比分析</h2></div>';
    ...
    for (var idx = 0; idx < historyData.length; idx++) {
        var h = historyData[idx];
        var prev = idx > 0 ? historyData[idx - 1] : null;
        var tChange = prev ? calculateChange(h.total, prev.total) : null;
        var rChange = prev && h.rankSchool && prev.rankSchool ? calculateChange(h.rankSchool, prev.rankSchool) : null;
        var bgStyle = h.isCurrent ? 'background:#eff6ff;' : '';
        var fontWeight = h.isCurrent ? 'bold' : 'normal';
        var badge = h.isCurrent ? '<span style="font-size:11px; background:#3b82f6; color:white; padding:2px 8px; border-radius:10px; margin-left:8px;">本次</span>' : '';
    ...
    if (latest.subjects && Object.keys(latest.subjects).length > 0) {
        ...
        var subjectEntries = Object.entries(latest.subjects);
        for (var m = 0; m < subjectEntries.length; m++) {
            var k = subjectEntries[m];
            if (k[0] && k[0] !== 'total') {
    ...
    if (historyData.length >= 2) {
        setTimeout(function() {
            var ctx = document.getElementById('historyTrendChart');
...
function initHistoryPatch() {
    ...
    var originalDoQuery = window.doQuery;
    ...
    window.doQuery = function() {
        ...
        setTimeout(async function() {
            ...
            var sch = document.getElementById('sel-school').value;
            var cls = document.getElementById('sel-class').value;
            
            console.log('📊 获取历史成绩数据...');
            var historyResult = await getHistoryComparisonData(name, cls, sch);
