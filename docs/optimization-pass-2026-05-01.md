...
- Security: Supabase publishable keys were removed from Worker source, migration script defaults, and `wrangler.jsonc`; production now expects `SUPABASE_REST_API_KEY` as a Cloudflare secret, with legacy env names still accepted as migration fallbacks.
