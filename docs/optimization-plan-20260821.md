...
### 2.1 构建/首屏加载策略优化 ✓ 首轮完成
- Vite 经典脚本不会自动形成有效 Rollup chunk，因此不盲目增加 `manualChunks` 配置。
- 普通桌面首屏关键模块预加载从 36 个收敛为 24 个；省流量/2G 等受限网络自动收敛为 8 个，移动端仍为 4 个。
- 其余模块继续由 idle prefetch 和登录过渡预热接管，桌面壳保留完整预热，避免首次交互被请求洪峰阻塞。
- **验证**: 登录性能契约、runtime 顺序、完整构建与模块 smoke。
