...
| 2026-06-26 | P1/P2 | Service Worker 对显式版本 JS/CSS 改为 CacheFirst；`/assets/css/*` 与 JS 对齐为 immutable；hybrid system_data 写入/删除改为 D1 与 Supabase 并发；静态响应启用正式 CSP 并保留 Report-Only，临时保留 Alpine 所需 `unsafe-eval` 兼容 | commits `c70db2cc` / `1d65be73`, `npm run build`, `npm run test:security-hygiene`, `npm run check:release-fast`, `npm run smoke:mobile:local`, `npm run smoke:modules:local`, `npm run smoke:prod-minimal`, production `node scripts/smoke-all-modules.js`, Cloudflare version `3eab6b2f-7700-4d09-b27e-43428850a213` |
