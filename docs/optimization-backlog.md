...
| 2026-08-27 | P1 | 成绩单历史查询中“未找到该学生/无权限查询”提示统一使用 `reportHistorySafeAlert` → `UI.alert`，保留学校别名识别、学生定位、权限判断与成绩单查询口径；新增 dialog 契约守卫 | `npm run validate`, `npm run test:dialog-runtime-contract`, `npm run test:report-performance-caches`, production `SMOKE_MODULE_IDS=student-details node scripts/smoke-all-modules.js`（部署后记录）, Cloudflare version（部署后记录） |
