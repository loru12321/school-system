...
## ★ 界面优化基准（2026-07-12，必须遵守）

后续所有界面和交互优化以云端分支 `origin/codex/ui-language-redesign` 的提交 `c9c1966a`（2026-07-12 06:41:23 +0800，`fix: keep mobile login above workbench`）为体验基准，**不得以其后的版本回退或弱化该版本的工作台和学生报告体验**。

- 学生报告对比表保持“三段式”信息结构：每个成绩与排名单元都明确显示“本次 / 上次 / 变化”。
- 历史数据尚未到达时保留当前真实排名和“暂无对比”，不得用“历史排名未归档”替代有效排名。
- 可在此基准上修复云端历史自动补齐、缓存失效重绘和响应性能，但不得改变成绩计算、排名口径、学校归一或数据结构。
...
**CSS `<link>` 顺序（src/index.html，末尾几层）：** …→ product-redesign → designer-studio-workspace → cloud-archive-visibility → mature-system-shell → workbench-design-language → **responsive-login-final（最后）**。旧 `editorial-control-system` / `operational-system` 层已删除，数据准备控制台的必要规则已收口到工作台层；登录最终层保持末位，以满足响应式登录契约。
