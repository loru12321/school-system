...
- 再复核：外部工具曾尝试把剩余 `app.js` 直接拆成 13 个普通 `app-*.js` 文件，并把 `app.js` 清空为入口存根；该方案已判定不适合当前系统并回退。原因是仓库现有构建、运行顺序和契约测试仍以 `app.js` 作为主运行时锚点，且当前模块化约定是 `*-runtime.js` 小步拆分，不是一次性替换主文件。
...
> 当前建议：以下项目只适合按“一个功能块一个 `*-runtime.js`”的小步方式继续，并且每提取一个功能块都必须同步更新对应契约测试与加载顺序。不要再创建普通 `app-*.js` 文件，也不要把 `app.js` 清空为存根。
