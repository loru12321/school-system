...
<<<<<<<< HEAD:public/assets/js/boot-runtime-runtime-f65c12832149.js
var BOOT_ASSET_VERSION_FALLBACK = 'runtime-f65c12832149';
========
var BOOT_ASSET_VERSION_FALLBACK = 'runtime-100163901213';
>>>>>>>> 461b3471 (fix: unify threshold resolution and source metadata):public/assets/js/boot-runtime-runtime-100163901213.js

// 每次进入都重新验证账号密码和届别，仅清理当前会话/工作区身份。
(function forceFreshLoginEntry() {
    if (window.EMBEDDED_DB) return;
    try {
        window.EdgeGateway?.clearSession();
...
const bootScript = (currentScript && /boot-runtime(?:-runtime-[0-9a-f]{12})?\.js/i.test(String(currentScript.src || '')))
    ? currentScript
    : scripts.slice().reverse().find((script) => /boot-runtime(?:-runtime-[0-9a-f]{12})?\.js/i.test(String(script.src || '')));
...
window.__AUTH_READY_TIMEOUT_ID__ = window.setTimeout(() => {
    ...
    try {
        ...
        const inputEl = document.querySelector('input[type="text"], input[type="password"]');
        const inputVal = inputEl ? (inputEl.value ? '(non-empty)' : '(empty)') : '(no input found)';
        console.warn(
            '[boot-runtime] auth timeout after', timeoutMs, 'ms —',
            'CURRENT_USER:', user ? 'set' : 'null',
            '| CURRENT_ROLE:', role || 'null',
            '| login input:', inputVal,
            '| __AUTH_READY__:', !!window.__AUTH_READY__
        );
    } catch (_) {
...
'teaching-workbench-cohort-runtime.js',
...
'excel-style-runtime.js',
'parse-rows-runtime.js',
...
const runWarmup = () => {
    // Keep the first interactive frame quiet. Deferred modules are useful after
    // the workbench is usable, but fetching/initializing them immediately after
    // auth competes with the first paint on slower school-office machines.
...
if (typeof window.requestIdleCallback === 'function') {
    window.requestIdleCallback(runWarmup, { timeout: 4200 });
    return;
}
window.setTimeout(runWarmup, 4200);
}
...
function restoreBootGatewaySession() {
    if (window.__FRESH_LOGIN_ENTRY__) return Promise.resolve();
